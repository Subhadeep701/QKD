...
class Protocol:
    ...
    def asymptotic_key_rate(self, use_ukf=True):
        """
        Compute asymptotic key rate.
        If use_ukf=False, Bob measures directly without UKF phase correction.
        Also returns phase tracking MSE and SNR before/after correction.
        """
        ...
        t = int(np.ceil(-np.log2(e_cor)))  # Verification hash output length
        GF = 2 ** self.q
        delta = self.alpha / (2 ** (self.p - 1))
        SNR_theory = (self.mu - 1) / Chi

        data_filename = f"data/N_{self.N}_n_bks_{self.n_bks}_SNR_{SNR_theory}_p_{self.p}.npz"
        pe_data_filename = f"data/PE_N_{self.N}_n_bks_{self.n_bks}_SNR_{SNR_theory}_p_{self.p}.npz"
        ...
        if self.is_data_loaded:
            ...
            X = np.empty(shape=[self.n_bks, self.N], dtype=np.float64)
            Y = np.empty(shape=[self.n_bks, self.N], dtype=np.float64)

            pilot_period = 8
            A_pilot = 1.5
            use_two_axis = True
            phase_var_per_symbol = 0.0008
            sigma_process = np.sqrt(phase_var_per_symbol)
            sigma_meas = np.sqrt(s_z)

            for blk in range(self.n_bks):
                Q_X, P_X = gg02.prepare_states(self.N, self.mu)

                # ---- Insert pilots ----
                is_pilot = np.zeros(self.N, dtype=bool)
                toggle = 0
                i = 0
                while i < self.N:
                    is_pilot[i] = True
                    if use_two_axis and (i + 1) < self.N:
                        is_pilot[i + 1] = True
                        if toggle == 0:
                            Q_X[i], P_X[i] = A_pilot, 0.0
                            Q_X[i + 1], P_X[i + 1] = 0.0, A_pilot
                            toggle = 1
                        else:
                            Q_X[i], P_X[i] = 0.0, A_pilot
                            Q_X[i + 1], P_X[i + 1] = A_pilot, 0.0
                            toggle = 0
                        i += pilot_period
                    else:
                        Q_X[i] = A_pilot
                ...
                Q_Y_raw, P_Y_raw, true_phase = gg02.transmit_states_phase(
                    self.N, Q_X, P_X, self.T, self.eta, s_z, phase_var_per_symbol, ar_coeff=0.99
                )

                meas_basis = np.random.randint(0, 2, size=self.N).astype(np.uint8)
                ...
                if use_ukf:
                    ...
                    def hx_ukf(x, ancillary=None):
                        phi = float(x[0])
                        qA, pA, g, bas = ancillary
                        q_rot = qA * np.cos(phi) - pA * np.sin(phi)
                        p_rot = qA * np.sin(phi) + pA * np.cos(phi)
                        return np.array([g * (q_rot if bas == 0 else p_rot)])

                    ukf = UKF(dim_x=1, dim_z=1, fx=fx_ukf, hx=hx_ukf, dt=1.0, points=points)
                    ukf.x = np.array([0.0])
                    ukf.P = np.array([[1e-2]])
                    ukf.Q = np.array([[sigma_process ** 2]])
                    ukf.R = np.array([[sigma_meas ** 2]])

                    phi_est = np.zeros(self.N, dtype=np.float64)
                    gain = np.sqrt(self.T * self.eta)

                    for k in range(self.N):
                        ukf.predict()
                        if is_pilot[k]:
                            meas_k = Q_Y_raw[k] if meas_basis[k] == 0 else P_Y_raw[k]
                            ancillary = (Q_X[k], P_X[k], gain, int(meas_basis[k]))
                            ukf.update(np.array([meas_k]), ancillary=ancillary)
                        phi_est[k] = float(ukf.x[0])

                    # Apply correction
                    cos_phi = np.cos(-phi_est)
                    sin_phi = np.sin(-phi_est)
                    Q_Y_corr = Q_Y_raw * cos_phi - P_Y_raw * sin_phi
                    P_Y_corr = Q_Y_raw * sin_phi + P_Y_raw * cos_phi
                    meas_vals = np.where(meas_basis == 0, Q_Y_corr, P_Y_corr).astype(np.float32)

                    # Extra metrics
                    phase_mse = np.mean((phi_est - true_phase) ** 2)
                    signal_var = self.mu - 1  # Alice's modulation variance (theoretical)
                    noise_var_raw = np.var(np.where(meas_basis == 0, Q_Y_raw, P_Y_raw) - X[blk])
                    noise_var_corr = np.var(meas_vals - X[blk])
                    snr_raw = signal_var / noise_var_raw if noise_var_raw > 0 else 0.0
                    snr_corr = signal_var / noise_var_corr if noise_var_corr > 0 else 0.0
                    phase_mse_all.append(phase_mse)
                    snr_raw_all.append(snr_raw)
                    snr_corr_all.append(snr_corr)


                else:
                    # --- No correction (direct measurement) ---
                    meas_vals = np.where(meas_basis == 0, Q_Y_raw, P_Y_raw).astype(np.float32)

                Y[blk] = meas_vals
                X[blk] = gg02.key_sifting(self.N, Q_X, P_X, meas_basis)
        ...
        return {
            "R_m": R_m,
            "phase_mse": np.mean(phase_mse_all) if phase_mse_all else None,
            "snr_raw": np.mean(snr_raw_all) if snr_raw_all else None,
            "snr_corr": np.mean(snr_corr_all) if snr_corr_all else None,
        }