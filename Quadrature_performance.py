...
print("Error Metrics theta:")
print(f"UKF theta -> MSE: {mse_ukf:.6f}, MAE: {mae_ukf:.6f}")
print(f"EKF theta -> MSE: {mse_ekf:.6f}, MAE: {mae_ekf:.6f}")
print(f"EIKF theta-> MSE: {mse_eikf:.6f}, MAE: {mae_eikf:.6f}")


# ------------------- Measurement-Domain Evaluation -------------------
# Ideal (no phase noise)
qB_ideal = gain * q + zq
pB_ideal = gain * p + zp
...
q_corr_ukf, p_corr_ukf = correct_measurements(phi_est)
q_corr_ekf, p_corr_ekf = correct_measurements(phi_est_ekf)
q_corr_eikf, p_corr_eikf = correct_measurements(phi_est_eikf)

mse_no_phase = np.mean((q - qB_ideal/gain)**2 + (p - pB_ideal/gain)**2)
mse_corr_ukf = np.mean((q - q_corr_ukf)**2 + (p - p_corr_ukf)**2)
mse_corr_ekf = np.mean((q - q_corr_ekf)**2 + (p - p_corr_ekf)**2)
mse_corr_eikf = np.mean((q - q_corr_eikf)**2 + (p - p_corr_eikf)**2)

print("\nMeasurement-Domain MSE (Alice vs Bob corrected):")
print(f"Ideal (no phase rotation): {mse_no_phase:.6f}")
print(f"UKF corrected:            {mse_corr_ukf:.6f}")
print(f"EKF corrected:            {mse_corr_ekf:.6f}")
print(f"EIKF corrected:           {mse_corr_eikf:.6f}")



# ------------------- MSE over time -------------------
mse_ukf_time = (q - q_corr_ukf)**2 + (p - p_corr_ukf)**2
mse_ekf_time = (q - q_corr_ekf)**2 + (p - p_corr_ekf)**2
mse_eikf_time = (q - q_corr_eikf)**2 + (p - p_corr_eikf)**2
mse_raw_time = (q - qB_ideal/gain)**2 + (p - pB_ideal/gain)**2

plt.figure(figsize=(12,5))
plt.plot(mse_raw_time, label='Raw (no phase correction)', alpha=0.7)
plt.plot(mse_ukf_time, label='UKF corrected', alpha=0.7)
plt.plot(mse_ekf_time, label='EKF corrected', alpha=0.7)
plt.plot(mse_eikf_time, label='EIKF corrected', alpha=0.7)
plt.xlabel('Symbol index')
plt.ylabel('Instantaneous MSE')
plt.title('Measurement-Domain MSE over Time')
plt.legend()
plt.grid(True)
plt.show()

# ------------------- Average MSE bar plot -------------------
methods = ['Raw', 'UKF', 'EKF', 'EIKF']
avg_mse = [np.mean(mse_raw_time), np.mean(mse_ukf_time), np.mean(mse_ekf_time), np.mean(mse_eikf_time)]

plt.figure(figsize=(8,5))
plt.bar(methods, avg_mse, color=['gray', 'blue', 'green', 'red'])
plt.ylabel('Average MSE')
plt.title('Average Measurement-Domain MSE Comparison')
plt.grid(axis='y')
plt.show()
