...
@njit(fastmath=True, parallel=True, cache=True)
def transmit_states_phase(n, q, p, t, h, s, phase_var, ar_coeff=0.99):
    """
    Coherent states transmitted through a thermal-loss channel with AR(1) phase noise.
    :param n: number of coherent states
    :param q, p: Alice's quadratures
    :param t: channel transmittance
    :param h: setup efficiency
    :param s: Gaussian noise variance
    :param phase_var: innovation variance for the AR(1) process
    :param ar_coeff: AR(1) coefficient (0 < ar_coeff < 1)
    :return: Bob's q, p quadratures, and true phase trajectory
    """
    qB = np.empty(n, dtype=np.float64)
    pB = np.empty(n, dtype=np.float64)
    ...
    for i in range(n):
        ...
        qB[i] = np.sqrt(t * h) * q_rot + zq[i]
        pB[i] = np.sqrt(t * h) * p_rot + zp[i]
