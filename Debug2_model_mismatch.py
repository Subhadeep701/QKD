...
args.N_E = 3000   # training sequences
...
for measurement_noise_variance in meas_var_list:
    ...
    for phase_noise_variance in phase_var_list:
        ...
        sys_model = SystemModel(f=f, Q=Q, h=h_true, R=R,
                                T=args.T, T_test=args.T_test,
                                m=m, n=n,noise_type='pink')
        ...
        # ============================================================
        sys_model2 = SystemModel(f=f, Q=Q2, h=h, R=R2,
                                 T=args.T, T_test=args.T_test,
                                 m=m, n=n,noise_type='pink')
        sys_model2.InitSequence(m1x_0, m2x_0)
        KNet = KalmanNetNN()
        KNet.NNBuild(sys_model2, args)

        pipe = Pipeline_EKF(now_string(), "KNet", "KNet")
        pipe.setssModel(sys_model2)
        pipe.setModel(KNet)
        pipe.setTrainingParams(args)

        pipe.NNTrain(sys_model, cv_input, cv_target,
                     train_input, train_target, path_results='KNet/')
        _, _, _, KNet_out, _ = pipe.NNTest(sys_model2, test_input, test_target, path_results='KNet/')
        ...
        with torch.no_grad():
            ...
            # ==========================
            hom_kn, alice_kn, quad_kn = homodyne_measure(phi_kn_payload, meas_payload, alice_payload, measurement_noise_variance, rng)
            hom_rnn, alice_rnn, quad_rnn = homodyne_measure(phi_rnn_payload, meas_payload, alice_payload, measurement_noise_variance, rng)
            hom_ukf, alice_ukf, quad_ukf = homodyne_measure(phi_ukf_payload, meas_payload, alice_payload, measurement_noise_variance, rng)

            # ==========================
            # Compute key rates
            # ==========================
            I_AB_kn, eps_kn, chi_BE_kn, K_kn = compute_key_rate_exact(hom_kn, alice_kn, quad_kn, Vmod,measurement_noise_variance, A_chan)
            I_AB_rnn, eps_rnn, chi_BE_rnn, K_rnn = compute_key_rate_exact(hom_rnn, alice_rnn, quad_rnn, Vmod, measurement_noise_variance,
                                                                          A_chan)
            I_AB_ukf, eps_ukf, chi_BE_ukf, K_ukf = compute_key_rate_exact(hom_ukf, alice_ukf, quad_ukf, Vmod, measurement_noise_variance,
                                                                          A_chan)
