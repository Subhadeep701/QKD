...
import random
import time
from KalmanNet_TSP.Plot import Plot_extended
from torch.serialization import add_safe_globals
from KalmanNet_TSP.KNet.KalmanNet_nn import KalmanNetNN

# Allowlist the KalmanNetNN class for unpickling
add_safe_globals([KalmanNetNN])
...
class Pipeline_EKF:
    ...
    def NNTrain(self, SysModel, cv_input, cv_target, train_input, train_target, path_results, \
        ...
        for ti in range(0, self.N_steps):
            ...
            for t in range(0, SysModel.T):
                x_out_training_batch[:, :, t] = self.model(torch.unsqueeze(y_training_batch[:, :, t],2)).squeeze(-1)
            ...
            with torch.no_grad():
                ...
                for t in range(0, SysModel.T_test):
                    x_out_cv_batch[:, :, t] = self.model(torch.unsqueeze(cv_input[:, :, t], 2)).squeeze(-1)
    ...
    def NNTest(self, SysModel, test_input, test_target, path_results, MaskOnState=False,\
     randomInit=False,test_init=None,load_model=False,load_model_path=None,\
        ...
        self.model.NNBuild(SysModel, self.args)  # rebuild layers like in training
        # load state_dict
        if load_model:
            self.model.load_state_dict(torch.load(load_model_path, map_location=self.device))
        else:
            self.model.load_state_dict(torch.load(path_results + 'best-model.pt', map_location=self.device))
        ...
        for t in range(0, SysModel.T_test):
            x_out_test[:,:, t] = self.model(torch.unsqueeze(test_input[:,:, t],2)).squeeze(-1)
